...
def document_xml() -> str:
    completed_rows = [
        ...
        ["Admin-only CRUD",
         "Completed",
         "Admin users can now add, edit, and delete Members, Trainers, Workout Plans, and Equipment from their respective management screens through in-page overlay forms. Each mutation writes through DataStore to SQLite and updates the live list immediately. Staff users keep read-only search/list access and do not see the mutation controls; direct Staff access to //Reports redirects back to the Dashboard."],
    ]
    ...
    parts = [
        ...
        p("The Gymers project is a working .NET MAUI iOS app with a Mac Catalyst secondary target for fast local verification. Six core tab screens (Login, Dashboard, Members, Payments, Attendance, Reports) plus three route-only screens (Trainers, Workouts, Equipment) are implemented and persist their state in a SQLite-backed DataStore. Tapping any row in Recent Payments generates a one-page PDF receipt via UIKit's native PDF renderer; the Reports tab generates Revenue, Attendance, and Member Roster reports as multi-page PDF or CSV via the system share sheet. Role-based access control persists the Admin/Staff role from login through navigation, reflowing the bottom tab bar, hiding admin-only Dashboard analytics for Staff, guarding Reports from direct Staff route access, and making the four management modules read-only for Staff. Admin now has full create, edit, and delete controls for Members, Trainers, Workout Plans, and Equipment. Attendance offers a simulated QR/ID scan check-in alongside name search, the Dashboard surfaces expiring-membership alerts, and every trainer row + the Coach Spotlight card surfaces a weekly schedule. The build succeeds with 0 warnings and 0 errors on both iOS and Mac Catalyst. Every README scope item is implemented; remaining work is broader test coverage and visual polish."),
